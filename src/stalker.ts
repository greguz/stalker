...
import { ChangeEvent, DeleteEvent, InsertEvent, ReplaceEvent, UpdateEvent } from './events'
...
export class Stalker extends EventEmitter {
  ...
  public create(collection: string, action: (_id: ObjectId, event: InsertEvent) => Promise<any>) {
  ...
  public delete(collection: string, action: (_id: ObjectId, event: DeleteEvent) => Promise<any>) {
    this.subscribe('delete', collection, [], action)
  }

  public insert(collection: string, action: (_id: ObjectId, event: InsertEvent) => Promise<any>) {
